...
enum schoolRegionCodes {
    B = "sen", E = "ice", C = "pen", F = "gen", 
    G = "dje", D = "dge", I = "sje", H = "use", 
    J = "goe", K = "kwe", M = "cbe", N = "cne", 
    R = "gbe", S = "gne", P = "jbe", Q = "jne", 
    T = "jje"
};

function validateSchoolCodeRgion(prefix: string): asserts prefix is keyof typeof schoolRegionCodes {
    if (!(prefix in schoolRegionCodes)) {
        throw Error("존재하지 않는 지역입니다. 학교 코드 첫 번째 자리를 다시 확인해 주세요.");
    }
}
function validateNatSchoolCodeRegion(schoolCode: string): asserts schoolCode is keyof typeof nationalSchoolRegionCodes {
    if (!(schoolCode in nationalSchoolRegionCodes)) {
        throw Error("존재하지 않는 학교입니다. 학교 코드를 다시 확인해 주세요.");
    }
}
...
export default (
    ...
    (resolve, reject) => {
        ...
        const schoolTypeCode: string = {
            "elementary": "2",
            "middle": "3",
            "high": "4"
        }[schoolType];

        let schoolRegionCode: nationalSchoolRegionCodes|schoolRegionCodes;

        if (schoolCode[0] === 'A') {
            validateNatSchoolCodeRegion(schoolCode);
            schoolRegionCode = nationalSchoolRegionCodes[schoolCode];
        } else {
            validateSchoolCodeRgion(schoolCode[0]);
        ...
        request(url, (err, _, html) => {
            ...
            const table = Array<SchoolMenuTable>(); //식단을 저장하는 배열
